  ...
  return (
    <main className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-6 px-4 py-8 sm:px-6">
      <div>
        ...
        {/* Per-player logins have no real email (see createPlayerLogin) — their
            display name is the meaningful identity here. The shared viewer login
            has no linked player, so its real email is the only identity to show. */}
        <p className="text-sm text-grey-light">
          {session?.player ? session.player.display_name : session?.user.email}
        </p>
      </div>
