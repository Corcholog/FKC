...
import { championDisplayName, getChampionMap, getLatestVersion, type ChampionInfo } from "@/lib/ddragon";
import { formatKdaRatio, kdaRatioForGame } from "@/lib/format";
import { formatRole, formatRoleShort } from "@/lib/roles";
import { findOpponentBySlug, groupBySeries, loadScrimGames } from "@/lib/scrims/queries";
...
export default async function OpponentScoutingPage({
  ...
  const opponent = await findOpponentBySlug(supabase, slug);
  if (!opponent) notFound();

  const [games, version] = await Promise.all([
    loadScrimGames(supabase, { opponentId: opponent.id }),
    getLatestVersion(),
  ]);
