...
import type { TeamPickRow } from "@/lib/team/types";
import {
  cleanBanPlan,
  cleanText,
  validateSeries,
...
//
// Validation lives in lib/team/validate.ts — a "use server" module may only
// export async server functions, so keeping it here would make it untestable.

export type TeamActionResult = { error?: string; seriesId?: string };
...
/**
 * The columns of team_games an entered game owns.
 *
 ...
 */
function gameFields(game: TeamGameInput, gameNumber: number) {
  return {
...
/** The ten team_picks rows of one game. */
function pickRows(
  gameId: string,
  game: TeamGameInput,
...
export async function saveTeamSeries(input: TeamSeriesInput): Promise<TeamActionResult> {
  ...
  try {
    ...
    const { data: series, error: seriesError } = await supabase
      ...
      .insert({
        ...
        // Null on anything but an official — buildSeriesPayload drops them, and
        // validateSeries refuses a stage without a competition, so by the time a
        // value reaches here it is one somebody meant.
        competition_id: input.competitionId,
        stage: cleanText(input.stage, MAX_STAGE_CHARS),
    ...
    const { error: picksError } = await supabase.from("team_picks").insert(
    ...
    if (noteRows.length > 0) {
      const { error: notesError } = await supabase.from("team_game_notes").insert(noteRows);
      if (notesError) throw new Error(notesError.message);
    }

    revalidateTeamMatches();
    ...
    if (createdSeriesId && supabase) {
      await supabase.from("team_series").delete().eq("id", createdSeriesId);
    }
...
type PickBackup = TeamPickRow & { created_at: string };
 ...
 *
 * Deliberately *not* "delete it and save it again". team_game_notes hangs off
 * team_games.id with `on delete cascade`, so recreating the games would take
 ...
 */
export async function updateTeamSeries(
  seriesId: string,
  ...
  try {
    ...
    if (keptIds.length > 0) {
      const { error } = await supabase.from("team_picks").delete().in("game_id", keptIds);
      if (error) throw new Error(error.message);
    ...
    // saveTeamSeries' compensating rollback, in the only shape an update can
    ...
    if (supabase && dropped) {
      await supabase.from("team_picks").delete().in("game_id", dropped.gameIds);
      if (dropped.picks.length > 0) await supabase.from("team_picks").insert(dropped.picks);
      revalidateTeamMatches();
    }
...
export async function deleteTeamSeries(seriesId: string): Promise<TeamActionResult> {
  try {
...
export async function updateOpponentNotes(
  ...
  try {
    ...
    revalidatePath("/team/opponents", "page");
    revalidatePath("/team/opponents/[slug]", "page");
    return {};
  } catch (e) {
 ...
 * same reason `saveTeamSeries` is: a server action is a POST endpoint, so the
...
export async function addTeamGameNote(
  gameId: string,
  note: string,
...
export async function updateTeamGameNote(
  noteId: string,
  note: string,
): Promise<TeamActionResult> {
  try {
...
export async function deleteTeamGameNote(noteId: string): Promise<TeamActionResult> {
  try {
    const { supabase } = await requireSession();
    if (!noteId) return { error: "Missing note." };
 ...
 * revalidateTeamMatches(): a note changes no aggregate, so throwing away the drafts
...
function revalidateTeamNotes() {
  revalidatePath("/team/matches");
  revalidatePath("/team/matches/[id]", "page");
}
 ...
 */
function revalidateTeamMatches() {
  revalidatePath("/team");
  revalidatePath("/team/matches");
  revalidatePath("/team/drafts");
  revalidatePath("/team/opponents");
  revalidatePath("/team/opponents/[slug]", "page");
  revalidatePath("/team/matches/[id]", "page");
}
