// Turning a parsed replay into the form state the scrim entry form holds.
//
// Kept apart from lib/scrims/rofl.ts on the same line the rest of this folder
...
import { SCRIM_ROLES, type ScrimRole, type ScrimSide } from "@/lib/scrims/types";
import type { RoflPlayer, RoflReplay } from "@/lib/scrims/rofl";
 ...
 */
function byRole(players: RoflPlayer[]): Record<ScrimRole, RoflPlayer | null> {
  const slots = Object.fromEntries(SCRIM_ROLES.map((role) => [role, null])) as Record<
    ScrimRole,
    RoflPlayer | null
  >;
  ...
  for (const player of players) {
    const role = player.role as ScrimRole;
    if (SCRIM_ROLES.includes(role) && slots[role] === null) slots[role] = player;
...
export function fillGameFromReplay(
  ...
  const draft = (side: Record<ScrimRole, RoflPlayer | null>, ally: boolean) =>
    Object.fromEntries(
  ...
  const winner = SCRIM_ROLES.map((role) => ours[role]).find((p) => p !== null);
