...
import {
  activeScrimFilterCount,
  scrimFilterToQuery,
  type ScrimFilter,
  type ScrimFilterOptions,
} from "@/lib/scrims/filters";
...
export function TeamFilterBar({
  ...
  function go(next: ScrimFilter) {
    router.push(`${basePath}/scrims/team${scrimFilterToQuery(next)}`);
  }
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="flex flex-col gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) =>
              go({ ...filter, kind: value === ANY ? null : (value as ScrimFilter["kind"]) })
            }
          >
            <SelectTrigger className="h-8 w-full text-sm">
              <SelectValue>
                {(value: string) =>
                  value === ANY ? "All" : SCRIM_KIND_LABELS[value as keyof typeof SCRIM_KIND_LABELS]
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border pt-3">
        ...
        {active > 0 && (
          <Button
            ...
            size="sm"
            variant="ghost"
            onClick={() => router.push(`${basePath}/scrims/team`)}
          >
            <X />
            Clear
          </Button>
