...
import { applyScrimFilter, isEmptyScrimFilter, scrimFilterOptions, type ScrimFilter } from "@/lib/scrims/filters";
import { firstPickSplit, scrimDurationSplit } from "@/lib/scrims/team-splits";
import { overallRecord, type ScrimRecord } from "@/lib/scrims/team-stats";
import { groupBySeries } from "@/lib/scrims/queries";
import { SCRIM_ROLES, type ScrimGameView } from "@/lib/scrims/types";
import { ChampionIcon } from "@/components/champion-icon";
import { SectionCard } from "@/components/section-card";
import { ScrimGameCard, type PlayerLookup } from "@/components/scrims/draft-board";
import { TeamFilterBar } from "@/components/scrims/team-filter-bar";
import { WinrateRing } from "@/components/winrate-ring";
import { BarRow, MetaChip, winRateTone } from "@/components/scrims/scrim-ui";
import { cn } from "@/lib/utils";

// The team page — /scrims/team and its demo.
//
...
// ban lists, and the games themselves. Nothing here re-queries; `loadScrimGames`
// already returned the section's whole dataset and `applyScrimFilter` is a
// predicate over it.
...
function RecordLine({ label, record }: { label: string; record: ScrimRecord }) {
  return (
...
export function ScrimTeamView({
  ...
  const filtered = applyScrimFilter(games, filter);
  const options = scrimFilterOptions(games);

  const record = overallRecord(filtered);
  const draftOrder = firstPickSplit(filtered);
  const duration = scrimDurationSplit(filtered);
