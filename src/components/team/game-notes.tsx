...
// The review thread under a team match's draft.
//
...
// enforces. That split is deliberate and differs from the rest of the team
...
// lib/team/notes.ts). A tree of indents inside a board already carrying twenty
// champion portraits is unreadable by the third level on a phone.
//
// useTransition + toast rather than useActionState, matching the other team
...
function NoteBody({
  ...
  function save() {
    ...
    startBusy(async () => {
      const result = await updateTeamGameNote(note.id, text);
      if (result.error) {
  ...
  function remove() {
    startBusy(async () => {
      const result = await deleteTeamGameNote(note.id);
      if (result.error) {
        toast.error(result.error);
        return;
      }
          ...
          {replyTo && (
            <div className="flex flex-col gap-1.5">
              ...
              <NoteComposer
                ...
                onSubmit={async (text) => {
                  const result = await addTeamGameNote(gameId, text, replyTo.id);
                  if (result.error) {
                    toast.error(result.error);
                    return false;
                  }
    ...
    <section className="flex flex-col gap-2 border-t border-border pt-2">
      ...
      {composing ? (
        <NoteComposer
          ...
          onSubmit={async (text) => {
            const result = await addTeamGameNote(gameId, text);
            if (result.error) {
              toast.error(result.error);
              return false;
            }
