...
import { MAX_STAGE_CHARS } from "@/lib/team/validate";
import { patchFromVersion } from "@/lib/ddragon";
import type { CompetitionRow, TeamOpponentRow } from "@/lib/team/types";
...
import {
  ...
  type RosterOption,
} from "@/components/team/game-fields";
import {
  buildSeriesPayload,
  emptyGame,
  formSignature,
...
} from "@/components/team/draft-form-state";
import { ReplayDropzone, partitionReplays, sortReplaysByPlayOrder } from "@/components/team/replay-drop";
import { fillGameFromReplay, type ReplayContext } from "@/components/team/replay-prefill";
import { RoflError, readRoflFile, type RoflReplay } from "@/lib/team/rofl";
...
export function TeamSeriesForm({
  opponents,
  roster,
  defaultLineup,
  champions,
  ...
  defaultLineup: Record<TeamRole, string | null>;
  champions: PickableChampion[];
  version: string;
  /** Every tournament on record, for the competition field on an official. */
  ...
  const [initial] = useState<SeriesFormState>(() => ({
    ...
    notes: editing?.notes ?? "",
    // Prefilled with the newest competition rather than blank. An official is
    // almost always one of the tournament currently being played, and the patch
    // field is the precedent: an optional box beside nine required ones gets
    // skipped every time, which is how the patch column stayed empty for eight
    // games.
    competitionId: editing?.competitionId ?? competitions[0]?.id ?? "",
    stage: editing?.stage ?? "",
  ...
  const [kind, setKind] = useState<TeamMatchKind>(initial.kind);
  ...
  function removeGame(index: number) {
    // Dropping a saved game takes its note thread with it — team_game_notes
  ...
  function save() {
    ...
    startSaving(async () => {
      const result = editing
        ? await updateTeamSeries(editing.id, built.payload)
        : await saveTeamSeries(built.payload);
    ...
    <div className="flex flex-col gap-6">
      <div className="panel-hex flex flex-col gap-4 p-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="series-date">Date</Label>
            <Input
              id="series-date"
              type="date"
              value={playedOn}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={kind} onValueChange={(value) => setKind(value as TeamMatchKind)}>
              <SelectTrigger id="series-kind" className="w-full">
                <SelectValue>{(value: TeamMatchKind) => TEAM_MATCH_KIND_LABELS[value]}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                {TEAM_MATCH_KINDS.map((k) => (
                  <SelectItem key={k} value={k}>
                    {TEAM_MATCH_KIND_LABELS[k]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          {/* Only on an official, because a competition on a scrim is a claim
              nobody can act on — the tournament filter has to be able to trust
              that a game tagged LIDE 2 was actually played in it. The values
              survive switching away and back; buildSeriesPayload drops them. */}
          {kind === "official" && (
            <>
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="series-competition">Competition</Label>
                <Select
                  value={competitionId}
                  onValueChange={(value) => setCompetitionId(value as string)}
                >
                  <SelectTrigger id="series-competition" className="w-full">
                    <SelectValue>
                      {(value: string) =>
                        competitions.find((c) => c.id === value)?.name ?? "None"
                      }
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {/* An official against a team outside any tracked
                        competition is a real thing — a qualifier, a showmatch —
                        so "none" stays reachable. */}
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="series-stage">Stage</Label>
                <Input
                  id="series-stage"
                  value={stage}
                  onChange={(e) => setStage(e.target.value)}
                  maxLength={MAX_STAGE_CHARS}
                  placeholder="Fecha 3"
                />
              </div>
            </>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="series-fearless">Fearless</Label>
            <label
              htmlFor="series-fearless"
