// How the team is doing in scrims: overall, by side, by opponent, per player.
//
// Pure functions over ScrimGameView[], same shape as every soloq stat module —
...
export type ScrimRecord = Record_ & {
  losses: number;
  /** Rounded percentage, 0 when nothing has been played. */
  winRate: number;
};

export function toRecord(games: number, wins: number): ScrimRecord {
  return {
    games,
    wins,
    losses: games - wins,
...
export function overallRecord(games: ScrimGameView[]): ScrimRecord {
  return toRecord(games.length, games.filter((g) => g.win).length);
}

export function recordBySide(games: ScrimGameView[]): Record<ScrimSide, ScrimRecord> {
  const on = (side: ScrimSide) => overallRecord(games.filter((g) => g.side === side));
  return { blue: on("blue"), red: on("red") };
}

export function recordByKind(games: ScrimGameView[]): Array<{ kind: ScrimKind; record: ScrimRecord }> {
  const byKind = new Map<ScrimKind, Record_>();
...
export function recordByOpponent(games: ScrimGameView[]): OpponentRecord[] {
...
function emptyAgg(playerId: string | null, name: string): ScrimPlayerAgg {
  return {
    playerId,
    name,
    games: 0,
...
export function aggregateAllyPlayers(
  ...
  const byKey = new Map<string, ScrimPlayerAgg>();
 ...
 * `championWinRate` / `championKdaRatio` work on scrims unchanged.
 *
 * `damage_dealt_to_champions` is 0 because scrims don't record damage — nothing
