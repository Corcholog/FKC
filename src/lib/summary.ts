...
/** One roster member, with the name the prompt is allowed to use already chosen. */
export type TeamPromptPlayer = {
  name: string;
  tier: string | null;
  division: string | null;
  league_points: number | null;
...
export type TeamPromptData = {
  /** Best rank first. */
  roster: TeamPromptPlayer[];
  duos: { a: string; b: string; games: number; winRate: number }[];
  civilWars: { a: string; b: string; aWins: number; games: number }[];
  /** Only streaks long enough to be worth a sentence, longest win first. */
...
/**
 * The group-level splits both recaps are written from — duos, civil wars, who's
 * on a heater — since a summary that just lists five individual records adds
 * nothing over the award tiles above it.
 *
 * **Names are resolved here, once.** Pass `aliases` and every name in the
 * returned data is an alias, with unaliased players dropped entirely rather than
 * falling back to a real one — the same inner-join rule `demo_players` follows,
 * so adding a roster member hides them from the demo until somebody aliases
 * them. The prompt builders never see an id, so the public profile in
 * summary-analyst.ts cannot reach a display name even by mistake.
 *
 * Returns null when there is nothing to summarise.
 */
export async function gatherTeamPromptData(
  supabase: SupabaseClient,
  options: { aliases?: AliasMap } = {},
): Promise<TeamPromptData | null> {
  ...
  // Anonymised: only aliased players exist at all. Un-anonymised: everybody, and
  // "alguien" covers an id the roster read didn't return.
  const nameById = new Map(
    allPlayers
      .map((p) => [p.id as string, aliases ? aliases.get(p.id as string) : (p.display_name as string)])
      .filter((entry): entry is [string, string] => entry[1] !== undefined),
  );
  const roster = allPlayers.filter((p) => nameById.has(p.id as string));
  const nameOf = (id: string) => nameById.get(id) ?? "alguien";
  /** A pairing is only usable if both halves have a name the prompt may say. */
  const known = (id: string) => !aliases || nameById.has(id);
  ...
  return {
    roster: [...roster]
      ...
      .map((p) => ({
        name: nameOf(p.id as string),
        tier: p.tier as string | null,
        division: p.division as string | null,
        league_points: p.league_points as number | null,
        wins: p.wins as number | null,
        losses: p.losses as number | null,
      })),
    duos: duoStats.duos
      .filter((d) => d.games >= MIN_DUO_GAMES && known(d.a) && known(d.b))
      .slice(0, 5)
      .map((d) => ({ a: nameOf(d.a), b: nameOf(d.b), games: d.games, winRate: duoWinRate(d) })),
    civilWars: duoStats.civilWars
      .filter((w) => known(w.a) && known(w.b))
      .slice(0, 5)
      .map((w) => ({ a: nameOf(w.a), b: nameOf(w.b), aWins: w.aWins, games: w.games })),
    streaks: [...streaks.entries()]
      .filter(([id, s]) => Math.abs(s.current) >= 3 && known(id))
      .sort(([, a], [, b]) => b.current - a.current)
      .map(([id, s]) => ({ name: nameOf(id), current: s.current })),
    weekGames: weekRows.length,
    weekWins: weekRows.filter((r) => r.win).length,
  };
}

/** The blocks both recap prompts share, formatted. Names only — no ids reach here. */
export function teamPromptBlocks(data: TeamPromptData) {
  return {
    roster: data.roster
      .map(
        (p) =>
          `- ${p.name}: ${formatRank(p.tier, p.division)}${
            p.tier ? ` (${p.league_points ?? 0} LP)` : ""
          }, ${formatWinLoss(p.wins, p.losses)} (${formatWinRate(p.wins, p.losses)})`,
      )
      .join("\n"),
    duos: data.duos
      .map((d) => `- ${d.a} + ${d.b}: ${d.games} games together, ${d.winRate}% winrate`)
      .join("\n"),
    civilWars: data.civilWars
      .map(
        (w) =>
          `- ${w.a} vs ${w.b} (opposite teams): ${w.aWins}-${w.games - w.aWins} in favour of ${
            w.aWins * 2 > w.games ? w.a : w.b
          }`,
      )
      .join("\n"),
    streaks: data.streaks
      .map(
        (s) =>
          `- ${s.name} is on a ${Math.abs(s.current)}-game ${s.current > 0 ? "win" : "loss"} streak`,
      )
      .join("\n"),
  };
}
...
export async function generateTeamSummary(
  ...
  const data = await gatherTeamPromptData(supabase);
  if (data === null) return { notEnoughData: true };
...
${blocks.duos || "Nobody has duoed enough for this to mean anything yet."}

Games where two of them were matched against each other:
${blocks.civilWars || "No civil wars yet."}
