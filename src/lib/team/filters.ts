...
//
// Every other page in this section answers "how have we done, ever". This is the
// one that
...
//
// Pure and I/O-free like the rest of lib/team/. The filter is applied in
...
// season of hand-entered games, `loadTeamGames` already fetches all of it for
...
import { TEAM_MATCH_KINDS, type TeamGameView, type TeamMatchKind } from "@/lib/team/types";

export type TeamMatchFilter = {
  kind: TeamMatchKind | null;
...
export const EMPTY_TEAM_MATCH_FILTER: TeamMatchFilter = {
  kind: null,
  opponentSlug: null,
  patch: null,
  from: null,
...
export type TeamMatchFilterParams = {
  kind?: string;
  opponent?: string;
  patch?: string;
  from?: string;
...
export function parseTeamMatchFilter(params: TeamMatchFilterParams): TeamMatchFilter {
  const kind = TEAM_MATCH_KINDS.find((k) => k === params.kind) ?? null;
...
export function teamMatchFilterToQuery(filter: TeamMatchFilter): string {
...
export function isEmptyTeamMatchFilter(filter: TeamMatchFilter): boolean {
  return activeTeamMatchFilterCount(filter) === 0;
}

/** How many constraints are set. Drives the "Clear" affordance and the count beside it. */
export function activeTeamMatchFilterCount(filter: TeamMatchFilter): number {
  return (
    (filter.kind ? 1 : 0) +
...
function championIdsOnSide(game: TeamGameView, ally: boolean): Set<number> {
 ...
 */
export function applyTeamMatchFilter(
  games: TeamGameView[],
...
export type TeamMatchFilterOptions = {
  ...
  kinds: Array<{ kind: TeamMatchKind; games: number }>;
 ...
 */
export function teamMatchFilterOptions(games: TeamGameView[]): TeamMatchFilterOptions {
  const opponents = new Map<string, { slug: string; name: string; games: number }>();
  const patches = new Map<string, number>();
  const kinds = new Map<TeamMatchKind, number>();
  ...
  return {
    ...
    // TEAM_MATCH_KINDS order, not count order: it runs practice → official, which is
    // how anyone reading the row thinks about them.
    kinds: TEAM_MATCH_KINDS.flatMap((kind) =>
