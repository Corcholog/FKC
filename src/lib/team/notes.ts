// Notes written on team-match games.
//
...
// docs/migrations/013_team_game_notes.sql for the full reasoning.
//
...
type NoteRow = TeamGameNote & { game_id: string };
 ...
 * without a name — see TeamGameNotes.
 */
...
export type LabelledTeamGameNote = TeamGameNote & { authorLabel: string | null };
 ...
 */
export type ThreadedNote = LabelledTeamGameNote & {
  /**
...
export type TeamNoteThread = ThreadedNote & { replies: ThreadedNote[] };
 ...
 */
export function threadNotes(notes: LabelledTeamGameNote[]): TeamNoteThread[] {
 ...
 * of the pages in this section use.
 */
export async function notesByGame(
  supabase: SupabaseClient,
  gameIds: string[],
): Promise<Map<string, TeamGameNote[]>> {
  const byGame = new Map<string, TeamGameNote[]>();
