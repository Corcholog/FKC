...
import type { ChampionPickAgg } from "@/lib/team/draft-stats";
import { hasRiotTag, nicknameOf, type TeamGameView } from "@/lib/team/types";
 ...
 */
export function deriveOpponentRoster(games: TeamGameView[]): OpponentPlayer[] {
 ...
 */
export function namedPickCoverage(games: TeamGameView[]): { named: number; total: number } {
  let named = 0;
  let total = 0;
  for (const game of games) {
    for (const pick of game.picks) {
