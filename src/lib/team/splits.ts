...
// lib/side-stats.ts) already answer both questions; this adapts team-match rows to
...
//
//   1. `team_games.duration_seconds` is **nullable** — it is typed in by hand
//      and often skipped — and `team_games` has no 15-minute floor, unlike
...
import { toRecord, type TeamRecord } from "@/lib/team/stats";
import { hadFirstPick, type TeamGameView } from "@/lib/team/types";
...
export function teamDurationSplit(games: TeamGameView[]): TeamDurationSplit {
 ...
 */
export function firstPickSplit(games: TeamGameView[]): FirstPickSplit {
  let firstGames = 0;
  let firstWins = 0;
  let secondGames = 0;
  let secondWins = 0;
